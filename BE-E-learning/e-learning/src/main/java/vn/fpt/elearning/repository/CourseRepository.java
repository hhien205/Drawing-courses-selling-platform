...
public interface CourseRepository extends JpaRepository<Course, String>, JpaSpecificationExecutor<Course> {
    ...
    @Query(
        "SELECT new course.model.vn.fpt.elearning.CourseDto(c.id, c.courseName, c.description, c.price, c.imageUrl, c.status, c.approveStatus, c.createDate, t.id, CONCAT(ut.firstName, ' ', ut.lastName), COUNT(u.id), c.discountPercentage, c.price * (100 - c.discountPercentage) / 100, 0, COALESCE(avg(r.star), 0)) "
            + "FROM Course c " +
    ...
    @Query(value = "select new response.course.dtos.vn.fpt.elearning.CmsGetDetailCourseResponse(" +
